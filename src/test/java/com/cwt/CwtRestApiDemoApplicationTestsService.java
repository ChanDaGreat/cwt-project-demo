...
@Configuration
@SpringBootTest
class CwtRestApiDemoApplicationTestsService {
	...
	void saveCustomer() {
		...
		Mockito.when(service.createCustomerRecord(customer)).thenReturn(customer);
		Customer expectedCustomer = service.createCustomerRecord(customer);
...
//		Customer customer = new Customer((long)3,"Pablo","Escobar","drugmedown@gmail.com","manila",null);
		Mockito.when(service.findAllByCustomers(address)).thenReturn(
				Stream.of(
						new Customer
						((long)3,"Pablo","Escobar","drugmedown@gmail.com","manila",null)
						).collect(Collectors.toList()));
		assertEquals(1,service.findAllByCustomers(address).size());
	...
	void updateCustomer() {
		...
		Customer updated = service.createCustomerRecord(updateCustomer);
	...
	void updatePartialCustomer() {
		...
		Customer updated = service.updateCustoerRecord(customer.getCustId(),toUpdateCustomer);
